...
class BmiCubit extends Cubit<BmiState> {
  ...
  void changeSliderValue(double value) => emit(BmiInitial(
        height: value,
        weight: state.weight,
        age: state.age,
      ));

  void incrementWeight() => emit(BmiInitial(
        weight: state.weight + 1,
        height: state.height,
        age: state.age,
      ));

  void decrementWeight() => emit(BmiInitial(
        weight: state.weight - 1,
        height: state.height,
        age: state.age,
      ));

  void incrementAge() => emit(BmiInitial(
        age: state.age + 1,
        height: state.height,
        weight: state.weight,
      ));

  void decrementAge() => emit(BmiInitial(
        age: state.age - 1,
        height: state.height,
        weight: state.weight,
      ));
